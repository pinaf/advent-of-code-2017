...
public final class Day07Common<T> implements Challenge<T> {
    ...
    public Day07Common(final SolutionFunction<T> solution, final String input) {
        this(
            solution,
            Arrays.stream(input.split("\n"))
                .map(String::trim)
                .map(Day07Common::parse)
                .toArray(ProgramInput[]::new)
        );
    }

    Day07Common(final SolutionFunction<T> solution, final ProgramInput head, final ProgramInput... tail) {
        this(
            solution,
            Stream.concat(
                Stream.of(head),
    ...
    private StructureNode findRoot() {
        ...
        Optional<ProgramInput> next = Day07Common.next(inputs.values().stream(), nodes.keySet());
        while (next.isPresent()) {
            final ProgramInput current = next.get();
            current.children().forEach(children::remove);
            inputs.remove(current.name());
            nodes.put(current.name(), Day07Common.toNode(current, nodes));
            next = Day07Common.next(inputs.values().stream(), nodes.keySet());
        }
    ...
    private static ProgramInput parse(final CharSequence input) {
        final Matcher matcher = Pattern.compile(Day07Common.REGEX).matcher(input);
        if (!matcher.find()) {
