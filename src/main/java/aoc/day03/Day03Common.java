...
public final class Day03Common implements Challenge<Long> {
    ...
    public Long run() {
        ...
        while (!this.halt.test(this.state)) {
            this.iterate(length, Day03Common.RIGHT);
            if (this.halt.test(this.state)) {
                break;
            }
            this.iterate(length, Day03Common.UP);
            length++;
            if (this.halt.test(this.state)) {
                break;
            }
            this.iterate(length, Day03Common.LEFT);
            if (this.halt.test(this.state)) {
                break;
            }
            this.iterate(length, Day03Common.DOWN);
            length++;
        }
